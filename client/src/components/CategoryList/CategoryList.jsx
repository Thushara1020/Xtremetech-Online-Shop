...
const CategoryList = () => {
  const {categories,setCategorise} = useContext(AppContext);
  ...
  const deleteByCategoryId = async (categoryId) => {
      try {
          ...
          if (response.status === 204) {
              const updateCategorise = categories.filter(category => category.categoryId !== categoryId);
                setCategorise(updateCategorise);
